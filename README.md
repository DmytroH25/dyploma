...
Кожну точку `Tm` також можна змінити вручну в таблиці команд. Поля `Tm.x` та
`Tm.y` працюють так само, як `G.x` і `G.y`: під час набору можна ввести будь-яке
число, а після `Enter` або втрати фокуса поле калібрується до найближчої точки,
яка існує на поточній кривій. Одна й та сама точка не використовується для двох
різних команд, щоб дешифрування залишалося однозначним.
