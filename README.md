...
кривої T<sub>m</sub>, маскується точкою `kG`, передається як криптограма T<sub>x</sub> і
...
`Tₘ.x = 0` не використовуються для команд. Різні команди не використовують
...
- Після шифрування або дешифрування відкривається візуалізатор кривої над F<sub>p</sub>,
  де кожен етап алгоритму підсвічує відповідні точки T<sub>m</sub>, T<sub>k</sub>, T<sub>x</sub> або -T<sub>k</sub>.
